...
class Records {
	...
	public function listRecords(){
		...
		while ($record = $result->fetch_assoc()) {
			...
			$rows[] = $record['role'];
			$rows[] = '<button type="button" name="view" id="'.$record["id"].'" class="btn btn-primary btn-xs view">Look</button>';
	...
	public function updateRecord(){
		...
		if($this->id) {
			...
			SET name= ?, nis = ?, username = ?, password = ?, role = ?
			WHERE id = ?");
			...
			$stmt->bind_param("sisssi", $this->name, $this->nis, $this->username, $this->password, $this->role, $this->id);
	...
	public function addRecord(){
		...
		if($this->name) {
			...
			$stmt = $this->conn->prepare("
			INSERT INTO ".$this->recordsTable."(`name`, `nis`, `username`, `password`, `role`)
			VALUES(?,?,?,?,?)");
			...
			$stmt->bind_param("sisss", $this->name, $this->nis, $this->username, $this->password, $this->role);
